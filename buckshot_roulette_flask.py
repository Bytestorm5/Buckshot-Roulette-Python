...
- Live "board" view updates after each move; shows items, turns, statuses, shotgun length.
...
from rl import RLEngine, PolicyNet
import torch
...
MODEL_REGISTRY['rl_model_mirror_op'] = 'rl_model_mirror_op'
MODEL_REGISTRY['rl_model_random_op'] = 'rl_model_random_op'
MODEL_REGISTRY['rl_model_random_op_2'] = 'rl_model_random_op_2'

# Built-in controller choices
CHOICES = ["Human", "Random", "Dealer"] + list(MODEL_REGISTRY.keys())
from rl import ITEMS, MOVE_TYPES
def build_engine(kind: str, idx: int):
    ...
    if kind in MODEL_REGISTRY:
        state_dim = 97
        move_dim = len(MOVE_TYPES) + (4 + 1) + 2
        policy = PolicyNet(state_dim=state_dim, move_dim=move_dim, hidden=128).to("cpu")
        policy.load_state_dict(torch.load(kind, map_location="cpu"))

        rl = RLEngine(playing_as=idx, policy=policy, optimizer=optim.Adam(policy.parameters(), lr=1e-5),
                      gamma=0.99, entropy_coef=0.01, train_mode=False)
        return rl
    # default
    return make_random(idx)
...
class GameSession:
    ...
    def do_move(self, move: str, ad_target: Optional[int] = None):
        ...
        if isinstance(new_shotgun, list):
            self.shotgun = list(new_shotgun)
        else:
            # keep current shotgun if no list provided
            self.shotgun = list(self.shotgun)
        ...
        if len(self.shotgun) == 0:
            ...
            for i, eng in enumerate(self.engines):
                if hasattr(eng, "on_reload") and callable(getattr(eng, "on_reload")):
                    try:
            ...
            self.history.append({
                ...
                "res_pub": f"reloaded (live={live}, total={total})",
                "res_priv": f"reloaded (live={live}, total={total})",
            })
        ...
        <div class="players">
          {% for i in range(0, session_obj.player_count) %}
            <div class="player">
              ...
              <div class="muted">Charges: {{ charges[i] }} / {{ session_obj.start_charges }}</div>
      ...
      <div class="card" style="margin-top:16px;">
        ...
        <ul class="history">
          {% for h in session_obj.history|reverse %}
            {% if h.event == 'reload' %}
              <li>🔄 <em>Shotgun reloaded</em> <span class="muted">• new shells: {{ h.shotgun_left }} (live/total updated)</span></li>
            {% else %}
              <li><strong>P{{h.player}}</strong> → <code>{{h.move}}</code>{% if h.ad_target is not none %} (target {{h.ad_target}}){% endif %}
              <span class="muted">• result: {{ h.res_pub if h.res_pub is not none else '—' }}{% if h.res_priv is not none and h.res_priv != h.res_pub %} (priv: {{ h.res_priv }}){% endif %} • shells: {{h.shotgun_left}}</span></li>
            {% endif %}
          {% endfor %}
...
from flask import get_flashed_messages  # used in template
...
def game():
    ...
    ctx = {
        ...
        "charges": getattr(gs.env, "charges", [0]*gs.player_count),
